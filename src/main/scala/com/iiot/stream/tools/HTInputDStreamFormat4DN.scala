...
import com.iiot.stream.base.{DP4StateStatistics, DPList, RedisOperation}
...
/**
  ...
  */
object HTInputDStreamFormat4DN {
  ...
  def inputDStreamFormat(stream: DStream[(String, String)]): DStream[DP4StateStatistics] = {
    val resultJson = stream.map(x =>
      JSON.parseObject(x._2,classOf[DPList]))
  ...
  def dataTransform(x: DPList): ArrayBuffer[DP4StateStatistics] = {
    logger.info("the raw string is " + x.toString)
    var ts: Long = 0L
    val tid = x.getTid
    val dsn = x.getDsn
    var data = x.getData
    var result = ArrayBuffer[DP4StateStatistics]()
    var dn: String = "0"
    var compId = "0"
    var thingId = "0"
    for (metric <- data){
      ...
      if ((dn != "0") && (dn != null)) {
        //compId = DeviceNumber.fromBase64String(dn).getMetricIdLong.toString
        //thingId = DeviceNumber.fromBase64String(dn).getDeviceIdInt.toString
        val dp = new DP4StateStatistics()
        val deviceNumber = com.htiiot.resources.utils.DeviceNumber.fromBase64String(dn)
        deviceNumber.getComponentId
        deviceNumber.getThingId
        dp.setDn(dn)
        dp.setTs(ts)
        dp.setMetric(metricName)
